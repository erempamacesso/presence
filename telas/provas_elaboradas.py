...
# 👇 IMPORTAÇÃO RESTAURADA AQUI NO TOPO!
try:
    ...
    if res_m.data:
        ...
        for index, prova in df_provas.iterrows():
            ...
            with st.container(border=True):
                ...
                # 👇 O BLOCO DA COLUNA 3 FOI RESTAURADO AQUI!
                with c3:
                    st.write("")
                    # Só gera a prova se ela tiver questões cadastradas
                    if qtd_questoes > 0:
                        try:
                            # Busca as questões diretamente do Supabase
                            res_q = supabase.table("questoes").select("*").in_("id", questoes_ids).execute()
                            
                            if res_q.data and 'gerar_prova_word' in globals():
                                # Motor cria o documento
                                ...
                                st.button("📄 Baixar Prova", disabled=True, key=f"d_err_{prova['id']}", help="Erro ao carregar gerador", use_container_width=True)
                        except Exception as e:
                            st.caption("Erro ao gerar arquivo de impressão.")
                    else:
