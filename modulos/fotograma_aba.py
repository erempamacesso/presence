        ...
        try:
            if icd.is_valid_item(codigo):
                ...
                termos = {
                    ...
                    "Attention deficit": "TDAH",
                    "Mental retardation": "Def. Intelectual",
                    "Epilepsy": "Epilepsia",
                    "Specific": "Específicos",
                    "Developmental": "do Desenvolvimento",
                    "Hyperkinetic": "Hipercinéticos",
                    "Conduct": "Conduta",
        ...
        if res.data:
            ...
            for _, row in df.iterrows():
                with st.container(border=True):
                    ...
                    status_atual = str(row.get("status_atual", ""))
                    status_cor = (
                        "red" if "Evasão" in status_atual
                        else "orange" if "acompanhamento" in status_atual.lower()
                        else "green"
                    )
                    st.markdown(f"**Status Atual:** :{status_cor}[{status_atual}]")
        else:
...
def abrir_popup_ocorrencia(aluno_id, nome, supabase):
    ...
    try:
        res = (
            supabase.table("ocorrencias_disciplinares")
            .select("*")
            .eq("aluno_id", aluno_id)
...
def abrir_popup_foto(nome, url_img):
    ...
    if isinstance(url_img, str) and url_img.startswith("http"):
        ...
        st.error(f"URL da foto inválida: {url_img}")
...
def gerar_pdf_pendencias(turma, alunos_pendentes):
    ...
    # Em fpdf2, output() sem parâmetros retorna bytes, que é o esperado pelo Streamlit
    return pdf.output()
...
def exibir_fotograma(supabase):
    ...
    try:
        ...
        res_busca = (
            supabase.table("historico_busca_ativa")
            .select("aluno_id,status_atual")
            .in_("status_atual", ["Em acompanhamento", "Alerta"])
            .execute()
        )
        ...
        total_sem_foto_escola = sum(
            1 for a in res_todos_alunos.data if limpar_texto(a.get("nome")) not in mapa_fotos
        )
        ...
        if lista_turmas:
            ...
            if turma_sel:
                ...
                with col_btn1:
                    ...
                    st.download_button(
                        "🖨️ Imprimir Mapa Nominal (PDF)",
                        data=pdf_mapa,
                        file_name=f"Mapa_Sala_{turma_sel}.pdf",
                ...
                if alunos_sem_foto:
                    with col_btn2:
                        with st.popover(f"🚩 {len(alunos_sem_foto)} Fotos Pendentes nesta Turma", use_container_width=True):
                            ...
                            st.download_button(
                                "📄 Baixar Lista de Busca",
                                data=pdf_pend,
                                file_name=f"Busca_Fotos_{turma_sel}.pdf",
                                mime="application/pdf",
                                use_container_width=True,
                                type="primary",
                            )
                        ...
                        with cols[j]:
                            ...
                            if len(cores_borda) == 1:
                                ...
                                estilo_borda = (
                                    ...
                                    f"linear-gradient(135deg, {gradiente}); "
                            ...
                            if isinstance(url_img, str) and url_img.startswith("http"):
                                ...
                                img_html = (
                                    ...
                                    "justify-content:center; font-size:50px;'>👤</div>"
                                )
                            ...
                            num_botoes = sum([
                                1 if tem_foto else 0,
                                1 if is_aee else 0,
                                1 if is_busca else 0,
                                1 if is_ocorrencia else 0,
                            ])
