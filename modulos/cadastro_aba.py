    ...
    if st.session_state.aba_cadastro_ativa == "📸 Fotos e Turmas":
        ...
        if not alunos:
            ...
            for aluno in alunos:
                ...
                with c3:
                    mat_atual = aluno.get("numero_matricula") or ""
                    ...
                    if nova_mat != mat_atual and nova_mat.strip() != "":
                        if st.button(
                            "💾 Salvar Matrícula",
                            key=f"btn_mat_{aluno['id']}",
                            use_container_width=True,
                        ):
                            supabase.table("alunos").update(
                                {"numero_matricula": nova_mat.strip()}
                ...
                with c4:
                    try:
                        idx_atual = lista_turmas.index(aluno["turma"])
                    except:
                        idx_atual = 0
                    ...
                    if nova_t != aluno["turma"]:
                        supabase.table("alunos").update({"turma": nova_t}).eq(
                            "id", aluno["id"]
                        ).execute()
                ...
                with c6:
                    with st.popover("🗑️"):
                        ...
                        if st.button(
                            "Apagar", key=f"del_{aluno['id']}", type="primary"
                        ):
        ...
        with st.form("novo_aluno"):
            ...
            if st.form_submit_button("Salvar Aluno"):
                if nome and turma:
                    ...
                    if dt_nascimento:
                        dados_insert["data_nascimento"] = dt_nascimento.strftime(
                            "%Y-%m-%d"
                        )
                    if matricula:
                        dados_insert["numero_matricula"] = matricula.strip()
        ...
        if busca:
            ...
            try:
                ...
                res = (
                    supabase.table("alunos")
                    .select("*")
                    .or_(f"nome.ilike.%{busca}%,numero_matricula.ilike.%{busca}%")
                    .execute()
                )
            except Exception:
                ...
                try:
                    ...
                    st.error(
                        f"Erro interno de banco de dados. Verifique a tabela 'alunos'."
                    )
            ...
            if res and res.data:
                ...
                if "numero_matricula" not in df.columns:
                    df["numero_matricula"] = "---"
                ...
                st.dataframe(
                    df[["id", "nome", "numero_matricula", "turma", "data_nascimento"]],
                    use_container_width=True,
                    hide_index=True,
                )
