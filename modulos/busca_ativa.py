...
# ==========================================
# CORREÇÃO #6: Importar fonte com suporte UTF-8 (fpdf2)
# ==========================================
...
def listar_arquivos_bucket(_supabase):
    """
    CORREÇÃO #5: Adicionado log de erro para não engolir exceções silenciosamente.
    """
    try:
        arquivos = _supabase.storage.from_('fotos-alunos').list(
            path=None, options={'limit': 5000}
        )
        mapa = {}
        if arquivos:
            for arq in arquivos:
        ...
        # CORREÇÃO #5: Loga o erro em vez de engolir silenciosamente
        st.warning(f"⚠️ Não foi possível carregar fotos do bucket: {e}")
        return {}


def gerar_pdf_relatorio(df, titulo_relatorio, data_hoje):
    """
    CORREÇÃO #6: Uso de fonte DejaVu (built-in no fpdf2) com suporte completo
    a UTF-8, eliminando o problema de caracteres especiais (ç, ã, etc.)
    virarem '?'.
    """
    try:
        ...
        # CORREÇÃO #6: Usando fonte com suporte a UTF-8 nativa do fpdf2
        ...
        for turma in turmas:
            ...
            pdf.cell(
                0, 10, "RELATÓRIO DE FUGA DE AULA",
                new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C"
            )
            pdf.set_font("DejaVu", "", 10)
            pdf.cell(
                0, 5, f"Gerado em: {data_hoje}",
                new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C"
            )
            pdf.ln(5)

            pdf.set_font("DejaVu", "B", 12)
            titulo_safe = str(titulo_relatorio)
            if turma:
                titulo_tela = (
                    titulo_safe
                    if "Turma" in titulo_safe
                    else f"{titulo_safe} - Turma: {turma}"
                )
            else:
                titulo_tela = titulo_safe

            pdf.cell(
                0, 10, titulo_tela.upper(),
            ...
            if is_heatmap:
                ...
                cols_dados = [
                    c for c in colunas
                    if c not in ['Turma', 'Aluno', 'Total de Fugas']
                ]

                for data_col in cols_dados:
                    if curr_x + w_data > 195:
                        break
                ...
                for _, row in df_turma.iterrows():
                    ...
                    nome = str(row.get('Aluno', ''))[:32]
                    pdf.cell(w_aluno, 6, f" {nome}", border=1, align="L")

                    pdf.set_font("DejaVu", "B", 8)
                    pdf.cell(
                        w_total, 6,
                        str(row.get('Total de Fugas', 0)),
                        border=1, align="C"
                    )

                    for data_col in cols_dados:
                        if pdf.get_x() + w_data > 195:
                            break
...
def exibir_busca_ativa(supabase):
    st.markdown("""
        ...
        button[data-testid="stTab"][aria-selected="true"] {
            background-color: white; border-top: 4px solid #FF4B4B;
        ...
        try:
            ...
            if res_h.data:
                ...
                with col_t:
                    t_sel = st.selectbox(
                        "📍 Turma:",
                        ["Todas"] + sorted(df_h['turma'].unique().tolist()),
                        key="rk_t",
                    )
                with col_s:
                ...
                if not rk.empty:
                    # CORREÇÃO #3 (ranking): uso de last() no lugar de drop_duplicates
                    # para garantir que cada aluno tenha apenas uma turma associada,
                    # mesmo que apareça em registros de turmas diferentes.
                    df_ts = (
                        df_h.groupby('aluno_nome')['turma']
                        .last()
                        .reset_index()
                    )
                    rk = rk.merge(df_ts, left_on='Aluno', right_on='aluno_nome').drop(
                        'aluno_nome', axis=1
                    )
                    rk.rename(columns={'turma': 'Turma'}, inplace=True)

                    st.dataframe(
                        rk.sort_values(by=['Turma', 'Faltas'], ascending=[True, False]),
                        use_container_width=True,
                        hide_index=True,
                    )

                    pdf_r = gerar_pdf_relatorio(rk, "Ranking de Faltas", data_hora_atual)
                    if isinstance(pdf_r, bytes):
                        st.download_button(
                            "📄 Baixar PDF", pdf_r,
                            "ranking.pdf", "application/pdf",
                            use_container_width=True,
                        )
                    else:
                    ...
                    st.info("Nenhum aluno atingiu o mínimo de faltas selecionado.")
            else:
                st.info("Sem registros de faltas.")
        except Exception as e:
            st.error(f"Erro: {e}")
    ...
    with aba_zero:
        ...
        try:
            ...
            # CORREÇÃO #2: Filtra presenças SOMENTE do dia atual
            r_com_p = (
                supabase.table("frequencia")
                .select("aluno_nome")
                .eq("status", "P")
                .eq("data_chamada", hoje)   # <-- filtro de data adicionado
                .execute()
            )
            ...
            if r_todos.data:
                ...
                n_p = (
                    [x['aluno_nome'] for x in r_com_p.data]
                    if r_com_p.data
                    else []
                )
                ...
                if not df_z.empty:
                    ...
                    st.dataframe(
                        df_z.sort_values(by=['Turma', 'Aluno']),
                        use_container_width=True,
                        hide_index=True,
                    )
                    ...
                    if isinstance(pdf_z, bytes):
                        st.download_button(
                            "📄 Baixar Relatório", pdf_z,
                            "abandono.pdf", "application/pdf",
                            use_container_width=True,
                        )
                    else:
                        st.error("⚠️ Ocorreu um erro interno ao gerar o PDF.")
                    ...
                    st.success("Nenhum abandono detectado hoje.")
        except Exception as e:
            st.error(f"Erro: {e}")

    # ==========================================
    # ABA 3: MAPA DE INTENSIDADE
    # ==========================================
    with aba_lista:
        st.subheader("🗺️ Mapa de Intensidade de Evasões")

        col_i, col_f, col_t = st.columns([1, 1, 2])
        d_i = col_i.date_input(
            "Início",
            datetime.now(fuso).date() - pd.Timedelta(days=7),
        )
        ...
        try:
            ...
            if r_turmas.data:
                lista_turmas += sorted(
                    list(set([x['turma'] for x in r_turmas.data if x['turma']]))
                )
            ...
            res_e_mapa = (
                supabase.table("evasoes")
                .select("aluno_nome, turma, aula_periodo, data_registro")
                .gte("data_registro", d_i.strftime('%Y-%m-%d'))
                .lte("data_registro", d_f.strftime('%Y-%m-%d'))
                .execute()
            )
            ...
            if res_e_mapa.data:
                ...
                if not df_m.empty:
                    ...
                    # CORREÇÃO #3 (pivot): Agrega por data normalizada (sem hora)
                    # para evitar colunas duplicadas quando há múltiplos registros
                    # no mesmo dia com horários diferentes.
                    df_m['data_dt'] = df_m['data_dt'].dt.normalize()
                    ...
                    m_ev['Total de Fugas'] = m_ev.select_dtypes(
                        include=['number']
                    ).sum(axis=1)

                    m_ev = m_ev.reset_index()
                    m_ev.rename(
                        columns={'turma': 'Turma', 'aluno_nome': 'Aluno'},
                        inplace=True,
                    )
                    ...
                    m_ev = m_ev[ordem].sort_values(
                        by=['Turma', 'Total de Fugas'], ascending=[True, False]
                    )

                    st.markdown(
                        "**Legenda de Gravidade:** "
                        "🟢 `0 Fugas` | 🟡 `1 Fuga` | 🟠 `2 Fugas` | 🔴 `3+ Fugas`"
                    )

                    df_exibicao = (
                        m_ev.style
                        .format(lambda v: "" if v == 0 else v, subset=cols_datas)
                        .background_gradient(subset=cols_datas, cmap='YlOrRd')
                    )
                    st.dataframe(df_exibicao, use_container_width=True, hide_index=True)

                    titulo_pdf = "Relatório de Fuga de Aula"
                    if t_escolhida != "Geral (Todas as Turmas)":
                    ...
                    if isinstance(pdf_e, bytes):
                        ...
                        st.error(
                            "⚠️ Não foi possível gerar o PDF. "
                            "Tire um print do erro abaixo:"
                        )
                        st.code(pdf_e)
                else:
                    st.info(
                        f"Nenhuma evasão encontrada para "
                        f"'{t_escolhida}' neste período."
                    )
            else:
                st.success("Sem evasões no período selecionado.")
        except Exception as e:
            st.error(f"Erro: {e}")

    # ==========================================
    # ABA 4: REGISTRAR AÇÃO (placeholder)
    # ==========================================
    with aba_registro:
        st.info("Funcionalidade de registro de ações a ser implementada.")