    ...
    try:
            ...
            if st.button("🔍 Iniciar Cruzamento", type="primary"):
                ...
                if not df_bd.empty:
                    ...
                    for col in ["numero_matricula", "data_nascimento", "sexo"]:
                        df_final_idx[col] = df_final_idx[col].replace(
                ...
                mask_incompletos = df_final.apply(
                    lambda r: campo_vazio(r.get("numero_matricula"))
                ...
                m3.metric(
                    "🟡 Incompletos (Sem dados no Excel/Banco)", len(df_incompletos)
                )

                st.divider()
                st.success(
                    "Cruzamento finalizado! Avance para a Aba 3 para revisar, editar e sincronizar."
                )
        ...
        with tab_sincronismo:
            ...
            if not df_incompletos.empty:
                ...
                st.write(
                    "Os alunos abaixo não possuem Matrícula, Data de Nascimento ou Sexo nem no Excel nem no Banco. **Você pode digitar diretamente na tabela abaixo antes de salvar:**"
                )
                ...
                df_editado = st.data_editor(
                    ...
                    disabled=[
                        "nome",
                        "turma",
                    ],  # Bloqueia nome e turma para o usuário não quebrar a sincronização
                    hide_index=True,
            ...
            if not tem_mudanca and df_incompletos.empty:
                st.success(
                    "✅ O banco de dados já está 100% atualizado e sem nenhum dado faltando. Nenhuma ação necessária."
                )
                return

            st.subheader("Aplicar Alterações no SIGERPAM")
            st.error(
                "🔴 **ATENÇÃO:** Alunos transferidos serão removidos e os dados editados/mesclados serão salvos."
            )
            ...
            if confirmar:
                with st.spinner("Atualizando o Banco de Dados..."):
                    ...
                    if not df_editado.empty:
                        ...
                        df_final_idx.update(
                            df_editado_idx
                        )  # Substitui com os dados que o usuário digitou
                        df_final = df_final_idx.reset_index()
                    ...
                    registros_para_salvar = df_final[
                        ["nome", "turma", "numero_matricula", "data_nascimento", "sexo"]
                    ].to_dict("records")
                    ...
                    try:
                        ...
                        supabase.table("alunos").upsert(
                            registros_para_salvar, on_conflict="nome"
                        ).execute()
                    except Exception as e:
                    ...
                    if erros:
                        ...
                        st.success(
                            "🎉 Sincronização concluída com sucesso! Banco 100% atualizado."
                        )
                        st.balloons()
